object Q3_1 {
 ...
 def fgList(f: Int=>Int, g: Int=>Int, l: List[Int]): List[Int] = {
  ...
  def gx(g: Int => Int, l: List[Int], acc: List[Int]): List[Int] ={
    if(l.isEmpty) acc
    else gx(g, l.tail, acc ++ List(g(l.head)))
  }

  def fx(f: Int => Int, l: List[Int], acc: List[Int]): List[Int] = {
    if (l.isEmpty) acc
    else fx(f, l.tail, acc ++ List(f(l.head)))
  }
