...
object final22_2 {
  ...
  def partialMap(l:List[String])(f1:String => String)(f2: String => Boolean):List[String] ={
    partial(l, f1, f2, List())
  }

  def partial(l: List[String], f1: String => String, f2: String => Boolean, acc: List[String]): List[String] ={
    if(l.isEmpty) acc
    else if(f2(l.head)) partial(l.tail, f1, f2, acc ++ List(f1(l.head)))
    else partial(l.tail, f1, f2, acc ++ List(l.head))
  }
